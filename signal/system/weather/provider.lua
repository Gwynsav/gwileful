...
local url = 'wttr.in/' .. conf.area .. '?format=j1'
...
local function manage_output(out)
   if self.timer.started then return end

   if out:find('Unknown location') then
      gears.debug.print_warning('Set location unknown, stopping...')
      global_timer:stop()
      self.timer:stop()
      return
   end
